...
const postReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case actions.FILTER_POST_LIST:
      return actionHandler.filterListItem("posts", action.payload, {
        postLoading: false,
      });
    case actions.PUSH_IDEA:
      return actionHandler.unshiftItem(
        "posts",
        action.payload,
        actionHandler.unshiftItem("myPosts", action.payload, state)
      );
    ...
    case actions.REMOVE_SINGLE_POST:
      const singlePost = actionHandler.removeItem(
        "posts",
        (post) => {
      ...
      return actionHandler.updateItem(
        ...
        (post) => {
          if (post._id === action.postId)
            return actionHandler.updateItem(
              "dislike",
              action.isDisliked ? post.dislike + 1 : post.dislike - 1,
              actionHandler.updateItem(
                "dislikedAccounts",
                (accounts) => {
    ...
    case actions.GET_POST_COMMENT:
      return actionHandler.updateItem(
        ...
        (post) => {
          if (post._id === action.postId)
            return actionHandler.getListItem("comments", action.payload, {
              ...post,
      ...
      return {
        ...
        posts: state.posts.map((post) => {
          if (post._id === action.postid) {
            return {
              ...post,
              comments: action.payload.length
    ...
    case actions.CREATE_POST_COMMENT:
      return actionHandler.updateItem(
        ...
        (post) => {
          if (post._id === action.postId)
            return actionHandler.unshiftItem("comments", action.payload, {
              ...post,
        ...
        (post) => {
          if (post._id === action.postId)
            return actionHandler.updateItem(
              ...
              (comment) => {
                if (comment._id === action.commentId)
                  return actionHandler.getListItem("replies", action.payload, {
                    ...comment,
                    page: 0,
                    count: 10,
                    loadMore: action.payload.length >= 10,
